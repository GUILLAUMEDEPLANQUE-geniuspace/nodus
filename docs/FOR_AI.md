...
40b. **Ghost cerveau V8.** `GhostCortex` (TF-IDF-like + hashing-trick, pas BM25, couches world/evidence/belief), `GhostTribunal` (preuve ou refus, jamais belief), `GhostSynapse` (Hebbian), `GhostDecay`, `GhostDream` (`applied = false`), `GhostGrowth` (sans observation : rien). Staff : `/n/{slug}/ghost/cerveau`. Question factuelle du chat : tribunal avant stratégie. Pas d’ingest à chaque tour public.
