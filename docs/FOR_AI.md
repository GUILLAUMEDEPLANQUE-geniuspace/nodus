...
15. Marque société = Geniuspace. Produit surface = Nodus. Vera = jobboard. Lumen = galerie.
...
40b. **Ghost cerveau V8.** `GhostCortex` (TF-IDF-like + hashing-trick, pas BM25, couches world/evidence/belief), `GhostTribunal` (preuve ou refus, jamais belief), citations publiques via `Ghost::publicCitations` (pas de page staff, pas de belief). Staff : `/n/{slug}/ghost/cerveau`. Golden : `GhostGoldenTest`. Alignement Engine : `missing` + phrase « il vous manque la preuve … ». Pas d’ingest à chaque tour public.
...
46. **Boucle cognitive v7.** OBSERVE → SITUATION → MEMORY → HYPOTHESIS → PLAN → CRITIC → SIMULATE → DECIDE → AUTHORIZE → VERIFY → REFLECT → LEARN. Situation JSON (hard/soft/trade-off). 5 couches de mémoire. Croyance = P + preuves + decay. Critic attaque, ne juge pas. Simulateur : `observed = null`. Autonomie = matrice, plafond d’exécution 54. Skill `recover_failed_campaign` (préconditions, ACT = humain). LLM propose via `ingest`, jamais juge. Pas de page produit nouvelle.
47. **Porte publique.** Home = créer d’abord + cinq germes (`WorldGerms`). Catalogue entier sous `/create`. Chat public = `GhostHost::publicSurface` après `Ghost::reply` + `publicCitations`. Ne pas renvoyer growth / belief / critic / simulations / maturity au visiteur. Ne pas effacer une classe Ghost*. Staff : lab, gym, cerveau, editor, maturity. Voir `docs/FOCUS.md`.
