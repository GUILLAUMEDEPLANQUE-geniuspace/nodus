...
40b. **Ghost cerveau V8.** `GhostCortex` (BM25 + hashing-trick, couches world/evidence/belief), `GhostTribunal` (preuve ou refus, jamais belief), `GhostSynapse` (Hebbian), `GhostDecay`, `GhostDream` (`applied = false`), `GhostGrowth` (sans observation : rien). Staff : `/n/{slug}/ghost/cerveau`. Question factuelle du chat : tribunal, pas LLM.
